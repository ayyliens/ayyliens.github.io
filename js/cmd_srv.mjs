...
export const dirs = ld.LiveDirs.of(
  hd.dirRel(c.DIR_TARGET),

  // The build system copies `./static/*` into `./target`.
  hd.dirRel(c.DIR_STATIC),

  // The build system copies `./img` into `./target` as `./target/img`.
  hd.dirRel(`.`, /^(?:img)[/]/),

  // Useful for client importmap overrides that point to local files.
